...
KARIS_LOCAL_BIN="${INSTALL_DIR}/karis-local"
...
install_from_source() {
    ...
    cp cli/bin/karis-local.js "${KARIS_LOCAL_BIN}"
    cp cli/bin/karis-staging.js "${KARIS_STAGING_BIN}"
    chmod +x "${KARIS_BIN}"
    chmod +x "${KARIS_LOCAL_BIN}"
...
echo "    karis-local setup        # First-time configuration (local :8000)"
